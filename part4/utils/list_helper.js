...
const totalLikes = (bloglist) => {
    ...
    return bloglist.length === 0 ? 0 : bloglist.map(blog => blog.likes).reduce(totalLikesCallback)
}
...
const mostBlogs = (bloglist) => {
    ...
    return {author: hasMostBlogsAuthor, blogs: authors.filter(author => author === hasMostBlogsAuthor).length}
}
...
const mostLikes = (bloglist) => {
    ...
    const blogWithMostLikes = bloglist.filter(blog => blog.likes === mostLikesNum)[0]
